...
def _remove_nodes_in_areas_with_same_category(df: GeoDataFrame):
    ...
    # Note: the index of new geodatframe should have the same index as the old ones, if not here is the issue.
    ...
    for idx_node, node_row in node_df.iterrows():
        ...
        found_areas_bool: GeoSeries = area_df[model_constants_i.CL_GEOMETRY].within(
            node_row[model_constants_i.CL_GEOMETRY])  # Note: This could be wrong if yes use contains() instead
...
def _set_data_entry_from_default_value(curr_default_value: DefaultValueEntry,
...
def _save_non_calculated_non_attribute_data(data_entry, row):
    DF_CL_NAME: str
    for DF_CL_NAME in model_constants_i.DF_CL_REDUCTION_PHASE_WITHOUT_ATTRIBUTES:
        # Do point reduction
        if DF_CL_NAME == model_constants_i.CL_GEOMETRY:
...
def _calculate_activated_attributes(activated_attributes_dict, already_calculated_attributes,
...
def _add_calculated_data_to_data_saver(data_entry, reduction_phase_data):
...
def _add_attributes_to_data_saver(already_calculated_attributes, curr_default_value, data_entry,
...
def _parse_the_data_file(traffic_cell_file_path: Path,
    ...
    _remove_nodes_in_areas_with_same_category(df)

    # Initialize Data
    # --------------.
    # Initialize a dictionary in which we will save our calculated data
    reduction_phase_data = _initialize_data_save()
    ...
    for idx, row in df.iterrows():
        ...
        data_entry: Dict = _initialize_data_save()
        ...
        if curr_category.get_strictly_use_default_values():
            _set_data_entry_from_default_value(curr_default_value, reduction_phase_data, row, data_entry)
        ...
        # If strictly-use-default-values isn't activated, we need to calculate the data
            ...
            _add_attributes_to_data_saver(already_calculated_attributes, curr_default_value, data_entry,
                                          not_activated_attributes)
            ...
            _save_non_calculated_non_attribute_data(data_entry, row)
    ...
    def calculate(self, configuration_manager_o: ConfigurationManager,
        ...
        Args:
            configuration_manager_o (configuration_manager.ConfigurationManager): The object containing all the
                configuration required for an execution.
