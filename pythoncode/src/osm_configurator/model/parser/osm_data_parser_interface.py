...
class OSMDataParserInterface(ABC):
    """
    The OSMDataParser job, its to parse the OSMData into a human-readable format.
    ...
    def parse_osm_data_file(self, path):
        """
        It gets a path toward an OSM data in Protocolbuffer Binary Format(pbf) and transforms it into a
